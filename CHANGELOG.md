...
### Added: Configurable admin directory — rename `adm/` at setup time

**Core changes:**
- `setup.php` — New "Admin Directory" input field (default `adm`), stored as `$sys_adm_dir` in `inc/sys_admin.php`. If a non-default name is entered, the `adm/` directory is renamed during installation
- `tpl/adm_head.log` — Menu path now reads `$sys_adm_dir` instead of hardcoded `/adm/`
- `adm/sys.php` — Both `sys_admin.php` writers (config change + account management) now preserve `$sys_adm_dir`. Config tab displays the current admin directory (read-only)
- `inc/sys_admin.php` — New `$sys_adm_dir` parameter generated by setup

**Modified files:**
- `setup.php` — Added form field, validation, directory rename logic, `$sys_adm_dir` in `generateMainConfig()`
- `tpl/adm_head.log` — `$admPath` now uses `$sys_adm_dir` variable (line 18)
- `adm/sys.php` — Two writes of `sys_admin.php` now include `$sys_adm_dir`; config tab shows read-only directory info

### 新增：后台目录可配置化 — 安装时可重命名 `adm/` 目录

**核心变化：**
- `setup.php` — 新增「Admin Directory」输入框（默认 `adm`），存入 `inc/sys_admin.php` 的 `$sys_adm_dir` 参数。输入非默认值时，安装过程自动重命名目录
- `tpl/adm_head.log` — 菜单路径从硬编码 `/adm/` 改为读取 `$sys_adm_dir`
- `adm/sys.php` — 两处写入 `sys_admin.php` 的代码（改域名 + 账号管理）现在均保留 `$sys_adm_dir`；Config 标签页显示当前后台目录（只读）
- `inc/sys_admin.php` — 新增 `$sys_adm_dir` 参数，由 setup 生成

**修改文件：**
- `setup.php` — 新增表单字段、校验、目录重命名逻辑、`generateMainConfig()` 写入 `$sys_adm_dir`
- `tpl/adm_head.log` — `$admPath` 改为使用 `$sys_adm_dir` 变量（第 18 行）
- `adm/sys.php` — 两处写入 `sys_admin.php` 均补入 `$sys_adm_dir`；Config 标签页显示只读目录信息
